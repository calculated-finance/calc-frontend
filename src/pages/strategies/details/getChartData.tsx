...
export function getChartDataSwapsNew(
  events: StrategyEvent[] | undefined,
  fiatPrices: FiatPriceHistoryResponse['prices'] | undefined,
  includeLabel: boolean,
) {
...
export function getChartDataNew(
  events: StrategyEvent[] | undefined,
  fiatPrices: FiatPriceHistoryResponse['prices'] | undefined,
) {
  const completedEvents = getCompletedEvents(events);
  ...
  const chartData = eventsWithAccumulation?.map((event) => ({
    ...
    price: event.accumulation,
    // label: `$${(price[1] * findCurrentAmountInTime(price[0], eventsWithAccumulation)).toFixed(2)} (${new Date(
    //   price[0],
    // ).toLocaleTimeString()})`,
  }));
  return [...(getChartDataSwapsNew(events, fiatPrices, false) || [])];
}

export function getChartDataSwapsTraditional(
  events: StrategyEvent[] | undefined,
  fiatPrices: FiatPriceHistoryResponse['prices'] | undefined,
  includeLabel: boolean,
) {
  ...
  const chartData = eventsWithAccumulation?.map((event) => {
    ...
    return {
      date,
      price: Number((event.accumulation * currentPriceInTime * 0.5).toFixed(2)),
      label: includeLabel
        ? `Received ${Number(event.swapAmount.toFixed(4))} ${event.swapDenom} at ${date.toLocaleTimeString()}`
        : undefined,
    };
...
export function getChartDataTraditional(
  events: StrategyEvent[] | undefined,
  fiatPrices: FiatPriceHistoryResponse['prices'] | undefined,
) {
  const completedEvents = getCompletedEvents(events);
  ...
  return [...(getChartDataSwapsTraditional(events, fiatPrices, false) || [])];
}
