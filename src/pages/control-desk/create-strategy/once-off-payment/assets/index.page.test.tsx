...
const mockRouter = {
  isReady: true,
  push: jest.fn(),
  pathname: '/control-desk/create-strategy/once-off-payment/assets',
...
describe('Once off payment Assets page', () => {
  beforeEach(() => {
    jest.clearAllMocks();
    mockFiatPrice();
  ...
  describe('when initial denom is selected', () => {
    describe('and there are available funds', () => {
      it('should show available funds', async () => {
        ...
        const select = await waitFor(() => screen.getByLabelText(/Which asset will you deposit?/));
  ...
  describe('when form is filled and submitted', () => {
    it('submits form successfully', async () => {
      ...
      await waitFor(() => screen.getByText(/Which asset will you deposit?/));
      await selectEvent.select(screen.getByLabelText(/Which asset will you deposit?/), ['KUJI']);
      ...
      // set over-collateralised deposit amount
      ...
      expect(mockRouter.push).toHaveBeenCalledWith({
        pathname: '/control-desk/create-strategy/once-off-payment/customise',
        query: { chain: 'Kujira' },
      });
    });
  });
