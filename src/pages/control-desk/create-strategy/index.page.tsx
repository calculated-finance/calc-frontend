...
export function ControlDeskPanels({ name, icon, href, learnMoreHref, description, enabled }: ControlDeskStrategyProps) {
  return (
    <Stack direction={['row', null, null, 'column']} width={['full', null, null, 'auto']}>
    ...
    const ControlDeskStrategy = [
      {
        ...
        description:
          'Deposit an asset, use it to DCA into the asset you want to make the payment in and once the grant target amount is reached, get the remaining assets returned back to your treasury.',
  ...
  return (
    <Stack>
      <Stack pb={6}>
        ...
        <Spacer />
        <Text textStyle="md">Built for treasuries to pay salaries, grants, and swap tokens the calculated way.</Text>
        <Text textStyle="body">Choose an option that suits you and your treasury. </Text>
      </Stack>
      <Stack>
        <Heading size="md">Tools</Heading>
        <SimpleGrid gap={8} w="auto" columns={[1, 1, 1, 2]}>
